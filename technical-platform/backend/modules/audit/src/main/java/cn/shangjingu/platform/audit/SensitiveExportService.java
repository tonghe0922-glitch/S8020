...
@Service
public class SensitiveExportService {
    ...
    public ExportRequest advance(DatabaseSecurityContext actor, UUID id, int expectedVersion, String requestedStatus) {
        return transactions.required(actor, () -> {
            ...
            if ("S02".equals(requestedStatus)) policy().validate(PolicyStage.DATA_SCOPE, current);
                ...
                repository.enqueueGeneration(current, expectedVersion + 1, actor.userId());
            int updated = repository.updateStatus(
                    actor.tenantId(), id, expectedVersion, requestedStatus, null, actor.userId());
    ...
    public ExportRequest recordGenerated(
            DatabaseSecurityContext actor, UUID id, int expectedVersion, GenerationResult result) {
        Objects.requireNonNull(result, "result");
        if (result.fileId() == null
                || blank(result.watermarkText())
        ...
        return transactions.required(actor, () -> {
            ExportRequest current = required(actor.tenantId(), id);
            requireVersion(current, expectedVersion);
            if (!"S05".equals(current.status()))
                throw new ProcessRejectedException("generated export result is not legal for current state");
    ...
    public DownloadGrant issueAuditedDownload(DatabaseSecurityContext actor, UUID id, int expectedVersion) {
        return transactions.required(actor, () -> {
            ExportRequest current = required(actor.tenantId(), id);
            requireVersion(current, expectedVersion);
            if (!"S06".equals(current.status())
                    || current.fileId() == null
                    || current.expireAt() == null
                    || !current.expireAt().isAfter(Instant.now())
                    || blank(current.watermarkText())) {
            ...
            if (grant == null
                    || blank(grant.location())
                    || grant.expiresAt() == null
                    || !grant.expiresAt().isAfter(Instant.now())
                    || grant.expiresAt().isAfter(current.expireAt())) {
    ...
    public ExportRequest expireAndDestroy(DatabaseSecurityContext actor, UUID id, int expectedVersion) {
        return transactions.required(actor, () -> {
            ExportRequest current = required(actor.tenantId(), id);
            requireVersion(current, expectedVersion);
            if (!"S08".equals(current.status()))
                throw new ProcessRejectedException("sensitive export is not ready for expiry/destruction");
            if (current.fileId() == null)
                throw new ProcessRejectedException("sensitive export file reference is missing");
            download().destroy(current);
            int updated = repository.updateStatus(
                    actor.tenantId(), id, expectedVersion, "S09", Instant.now(), actor.userId());
    ...
    private ExportPolicyCapability policy() {
        if (policies.size() != 1)
            throw new ProcessRejectedException("sensitive export policy capability is unavailable or ambiguous");
        return policies.getFirst();
    }

    private AuditedDownloadCapability download() {
        if (downloads.size() != 1)
            throw new ProcessRejectedException("audited download capability is unavailable or ambiguous");
    ...
    private static void requireVersion(ExportRequest request, int expectedVersion) {
        if (request.versionNo() != expectedVersion)
            throw new ProcessRejectedException("sensitive export version conflict");
    }
    ...
    private static void validate(CreateCommand command) {
        ...
        if (blank(command.exportType())
                || command.dataScope() == null
                || command.fieldScope() == null
                || blank(command.purpose())
                || blank(command.approvalLevel())
    ...
    public interface ExportPolicyCapability {
        void validate(PolicyStage stage, ExportRequest request);
    }

    public interface SensitiveExportGenerator {
        String exportType();

        GenerationResult generate(ExportRequest request, List<ExportItem> items);
    }

    public interface AuditedDownloadCapability {
        DownloadGrant issue(ExportRequest request);

        void revoke(DownloadGrant grant);

        void destroy(ExportRequest request);
    }
