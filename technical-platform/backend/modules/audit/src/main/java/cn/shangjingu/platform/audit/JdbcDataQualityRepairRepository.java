...
public class JdbcDataQualityRepairRepository implements DataQualityRepairService.Repository {
    ...
    @Override
    public void insert(
            DataQualityRepairService.QualityIssue q, List<DataQualityRepairService.QualityItem> items, UUID actorId) {
        jdbc.update(
                """
    ...
    public List<DataQualityRepairService.QualityItem> items(UUID tenantId, UUID id) {
        return jdbc.query(
                ...
                """,
                (rs, n) -> new DataQualityRepairService.QualityItem(
                        rs.getInt("item_seq"),
    ...
    public int updateStatus(
            UUID tenantId, UUID id, int expectedVersion, String status, Instant actualEndAt, UUID actorId) {
        return jdbc.update(
                """
    ...
    public int approvePlan(UUID tenantId, UUID id, int expectedVersion, String planJson, UUID reviewerUserId) {
        insertItem(
                tenantId,
                id,
                new DataQualityRepairService.QualityItem(9001, "repair_plan", "approved", "修复方案", planJson),
                reviewerUserId);
        insertItem(
                tenantId,
                id,
                new DataQualityRepairService.QualityItem(
                        9002, "repair_reviewer_user_id", "reviewer", "复核人", reviewerUserId.toString()),
                reviewerUserId);
        return jdbc.update(
                """
                update audit.data_quality_issue
    ...
    public DataQualityRepairService.RepairControl repairControl(UUID tenantId, UUID id) {
        return jdbc.query(
                ...
                """,
                rs -> {
                    if (!rs.next() || rs.getString("reviewer_id") == null || rs.getString("plan_json") == null)
                        return null;
                    return new DataQualityRepairService.RepairControl(
                            UUID.fromString(rs.getString("reviewer_id")), rs.getString("plan_json"));
                },
                tenantId,
                id);
    }
    ...
    public int recordRepair(
        ...
        insertItem(
                ...
                id,
                new DataQualityRepairService.QualityItem(
                        9003, "repair_executor_user_id", "executor", "执行人", executorUserId.toString()),
                executorUserId);
        return jdbc.update(
                """
                update audit.data_quality_issue
    ...
    private static DataQualityRepairService.QualityIssue map(ResultSet rs) throws SQLException {
        return new DataQualityRepairService.QualityIssue(
                ...
                rs.getString("system_service_name"),
                rs.getString("tech_impact_scope"),
                rs.getString("tech_risk_level"),
