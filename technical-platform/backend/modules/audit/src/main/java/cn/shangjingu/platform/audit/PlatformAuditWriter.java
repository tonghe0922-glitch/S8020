...
public final class PlatformAuditWriter {
    ...
    public PlatformAuditWriter(JdbcTemplate jdbc, PlatformTransactionManager transactionManager) {
        if (jdbc == null || transactionManager == null)
            throw new IllegalArgumentException("audit writer dependencies are required");
        this.jdbc = jdbc;
    ...
    public UUID appendCriticalAccess(AccessCommand command) {
        ...
        if (result == null)
            throw new IllegalStateException("critical access audit transaction returned no evidence id");
        return result;
    }
    ...
    private static void validateOperation(OperationCommand c) {
        ...
        text(c.action(), 64, "action");
        text(c.resourceType(), 128, "resourceType");
        optional(c.requestId(), 128, "requestId");
    }
    ...
    private static void validateAccess(AccessCommand c) {
        ...
        text(c.resourceType(), 128, "resourceType");
        text(c.fieldsAccessedJson(), Integer.MAX_VALUE, "fieldsAccessedJson");
        optional(c.purpose(), 255, "purpose");
    }
    ...
    private static void validateSecurity(SecurityEventCommand c) {
        ...
        text(c.eventType(), 64, "eventType");
        text(c.severity(), 16, "severity");
        optional(c.ipAddress(), 64, "ipAddress");
        optional(c.deviceFingerprint(), 255, "deviceFingerprint");
        text(c.detailJson(), Integer.MAX_VALUE, "detailJson");
    }
    ...
    private static void optional(String value, int max, String field) {
    ...
    public record SecurityEventCommand(
            UUID tenantId,
            String eventType,
            String severity,
            UUID actorId,
