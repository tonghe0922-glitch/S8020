...
@Service
public final class DataQualityRepairService {
    ...
    public QualityIssue create(DatabaseSecurityContext actor, String idempotencyKey, String requestHash, CreateCommand command) {
        ...
        return transactions.required(actor, () -> {
            ...
            IdempotencyClaim claim = idempotency.claim(actor.tenantId(), actor.userId(), idempotencyKey, requestHash,
                    "audit.data_quality_issue", proposed, IDEMPOTENCY_TTL);
            if (claim.existing()) return required(actor.tenantId(), claim.resourceId());
            QualityIssue issue = new QualityIssue(
                    proposed, actor.tenantId(), numbers.next(actor.tenantId(), actor.userId(), PROCESS_CODE), STATES.initial(), 0,
                    command.ruleCode(), command.objectType(), command.objectId(), command.issueType(), command.severity(),
                    before, null, null, null, null, Instant.now(), null, command.businessDate(), command.employeeEventType(),
                    command.environment(), command.resultSummary(), command.systemServiceName(), command.techImpactScope(),
                    command.techRiskLevel(), actor.userId());
    ...
    public QualityIssue advance(DatabaseSecurityContext actor, UUID id, int expectedVersion, String requestedStatus) {
        return transactions.required(actor, () -> {
            ...
            if ("S03".equals(requestedStatus)) governance().validate(GovernanceStage.AUTHORITATIVE_SOURCE, current, null);
            else if ("S04".equals(requestedStatus)) governance().validate(GovernanceStage.IMPACT_ANALYSIS, current, null);
            else if ("S07".equals(requestedStatus)) compensation().compensate(current);
            int updated = repository.updateStatus(actor.tenantId(), id, expectedVersion, requestedStatus,
                    SequentialStateMachine.CLOSED.equals(requestedStatus) ? Instant.now() : null, actor.userId());
    ...
    public QualityIssue approvePlan(DatabaseSecurityContext actor, UUID id, int expectedVersion, Object plan) {
        if (actor.userId() == null || plan == null) throw new ProcessRejectedException("repair plan reviewer and plan are required");
    ...
    public QualityIssue executeRepair(DatabaseSecurityContext actor, UUID id, int expectedVersion) {
        ...
        return transactions.required(actor, () -> {
            ...
            int updated = repository.recordRepair(actor.tenantId(), id, expectedVersion, after,
                    result.resolutionAction(), actor.userId());
    ...
    public QualityIssue close(DatabaseSecurityContext actor, UUID id, int expectedVersion) {
        return transactions.required(actor, () -> {
            ...
            if (current.verifiedAt() == null || blank(current.afterSnapshotJson()) || blank(current.resolutionAction())) {
                throw new ProcessRejectedException("data-quality issue close evidence is incomplete");
            }
            int updated = repository.updateStatus(actor.tenantId(), id, expectedVersion,
                    SequentialStateMachine.CLOSED, Instant.now(), actor.userId());
    ...
    private RepairGovernanceCapability governance() {
        if (governance.size() != 1) throw new ProcessRejectedException("repair governance capability is unavailable or ambiguous");
        return governance.getFirst();
    }

    private RepairHandler handler(QualityIssue issue) {
        List<RepairHandler> matches = handlers.stream().filter(h -> h.supports(issue.objectType(), issue.issueType())).toList();
        if (matches.size() != 1) throw new ProcessRejectedException("repair handler is unavailable or ambiguous for issue type");
        return matches.getFirst();
    }

    private CompensationCapability compensation() {
        if (compensations.size() != 1) throw new ProcessRejectedException("repair compensation capability is unavailable or ambiguous");
        return compensations.getFirst();
    }

    private VerificationCapability verifier() {
        if (verifiers.size() != 1) throw new ProcessRejectedException("repair verification capability is unavailable or ambiguous");
        return verifiers.getFirst();
    }

    private QualityIssue required(UUID tenantId, UUID id) {
        return repository.find(tenantId, id).orElseThrow(() -> new ProcessRejectedException("data-quality issue not found"));
    }

    private static void requireVersion(QualityIssue issue, int expectedVersion) {
        if (issue.versionNo() != expectedVersion) throw new ProcessRejectedException("data-quality issue version conflict");
    }
    ...
    private static void validateCreate(CreateCommand c) {
        ...
        if (blank(c.ruleCode()) || blank(c.objectType()) || blank(c.issueType()) || blank(c.severity()) || c.beforeSnapshot() == null
                || c.businessDate() == null || blank(c.employeeEventType()) || blank(c.environment()) || blank(c.resultSummary())
                || blank(c.systemServiceName()) || blank(c.techImpactScope()) || blank(c.techRiskLevel())
                || c.items() == null) {
    ...
    public interface Repository {
        ...
        int recordRepair(UUID tenantId, UUID id, int expectedVersion, String afterSnapshotJson, String resolutionAction, UUID executorUserId);
        int recordVerified(UUID tenantId, UUID id, int expectedVersion, UUID actorId);
    }
    public interface RepairGovernanceCapability { void validate(GovernanceStage stage, QualityIssue issue, String planJson); }
    public interface RepairHandler { boolean supports(String objectType, String issueType); RepairResult execute(QualityIssue issue, String approvedPlanJson); }
    public interface CompensationCapability { void compensate(QualityIssue issue); }
    public interface VerificationCapability { void verify(QualityIssue issue); }
    public enum GovernanceStage { AUTHORITATIVE_SOURCE, IMPACT_ANALYSIS, PLAN_APPROVAL }
    ...
    public record CreateCommand(String ruleCode, String objectType, UUID objectId, String issueType, String severity,
            Object beforeSnapshot, LocalDate businessDate, String employeeEventType, String environment, String resultSummary,
            String systemServiceName, String techImpactScope, String techRiskLevel, List<QualityItem> items) {}
    public record QualityIssue(UUID id, UUID tenantId, String businessNo, String status, int versionNo, String ruleCode,
            String objectType, UUID objectId, String issueType, String severity, String beforeSnapshotJson,
            String afterSnapshotJson, String rootCause, String resolutionAction, Instant verifiedAt, Instant actualStartAt,
            Instant actualEndAt, LocalDate businessDate, String employeeEventType, String environment, String resultSummary,
            String systemServiceName, String techImpactScope, String techRiskLevel, UUID createdBy) {}
}
