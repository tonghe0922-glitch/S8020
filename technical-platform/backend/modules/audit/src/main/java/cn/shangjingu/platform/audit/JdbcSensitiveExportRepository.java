...
public class JdbcSensitiveExportRepository implements SensitiveExportService.Repository {
    ...
    public JdbcSensitiveExportRepository(
            JdbcTemplate jdbc, @Value("${platform.files.safe-virus-scan-status:CLEAN}") String safeStatus) {
        this.jdbc = jdbc;
    ...
    @Override
    public void insert(
            SensitiveExportService.ExportRequest r, List<SensitiveExportService.ExportItem> items, UUID actorId) {
        jdbc.update(
                """
    ...
    public void assertSafeFile(UUID tenantId, UUID fileId) {
        ...
        if (count == null || count != 1)
            throw new ProcessRejectedException("export file is missing or not virus-scan safe");
    }
    ...
    @Override
    public int recordGenerated(
            UUID tenantId, UUID id, int expectedVersion, SensitiveExportService.GenerationResult result, UUID actorId) {
        return jdbc.update(
                """
                update audit.data_export_request
    ...
    private static SensitiveExportService.ExportRequest map(ResultSet rs) throws SQLException {
        return new SensitiveExportService.ExportRequest(
                ...
                rs.getString("field_scope_json"),
                rs.getString("purpose"),
                rs.getString("approval_level"),
