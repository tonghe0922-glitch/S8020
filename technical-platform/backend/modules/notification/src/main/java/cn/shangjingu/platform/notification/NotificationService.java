...
public final class NotificationService {
    ...
    public NotificationService(JdbcTemplate jdbc, TransactionalOutboxService outbox, NotificationTemplateRenderer renderer) {
        if (jdbc == null || outbox == null || renderer == null) throw new IllegalArgumentException("notification dependencies are required");
        this.jdbc = jdbc;
        this.outbox = outbox;
    ...
    public UUID create(CreateCommand command) {
        ...
        jdbc.query("select pg_advisory_xact_lock(hashtextextended(cast(? as text),0))", rs -> { rs.next(); return null; }, lockMaterial);
        ...
        if (existing != null) {
            ...
            throw new NotificationConflictException("notification request key already exists with different message content");
        }
        ...
        int inserted = jdbc.update("""
                ...
                values (?,?,?,?,?,?,?,?,?,?,?,'PENDING',?)
                """, messageId, command.tenantId(), command.actorId(), command.actorId(), messageNo, template.id(),
                command.recipientType().trim(), command.recipientId(), command.channel().trim(), rendered.title(), rendered.body(),
                offset(scheduledAt));
        if (inserted != 1) throw new IllegalStateException("notification message insert failed");
        if (scheduledAt == null || !scheduledAt.isAfter(Instant.now())) enqueueSend(command.tenantId(), command.actorId(), messageId);
        return messageId;
    }

    public int enqueueDue(UUID tenantId, UUID actorId, int limit) {
        requireActiveTransaction();
        if (tenantId == null || limit <= 0 || limit > 500) throw new IllegalArgumentException("tenant and due-message limit are required");
        List<UUID> ids = jdbc.query("""
                ...
                limit ?
                """, (rs, rowNum) -> rs.getObject("id", UUID.class), tenantId, limit);
    ...
    private void enqueueSend(UUID tenantId, UUID actorId, UUID messageId) {
        outbox.enqueue(new TransactionalOutboxService.Command(
                tenantId, actorId, AGGREGATE_TYPE, messageId, SEND_EVENT_TYPE, 1,
                "{\"messageId\":\"" + messageId + "\"}", "notification-send:" + messageId));
    }
    ...
    private Template loadTemplate(UUID tenantId, String templateCode, String channel) {
        Template template = jdbc.query("""
                ...
                """, rs -> rs.next() ? new Template(rs.getObject("id", UUID.class), rs.getString("channel"),
                rs.getString("title_template"), rs.getString("body_template"), rs.getString("variables_schema")) : null,
                tenantId, templateCode.trim());
        if (template == null) throw new IllegalArgumentException("notification template not found or disabled");
        if (!template.channel().equals(channel.trim())) throw new IllegalArgumentException("notification template channel mismatch");
        return template;
    }
    ...
    private ExistingMessage findByMessageNo(UUID tenantId, String messageNo) {
        return jdbc.query("""
                ...
                """, rs -> rs.next() ? new ExistingMessage(
                rs.getObject("id", UUID.class), rs.getObject("template_id", UUID.class), rs.getString("recipient_type"),
                rs.getObject("recipient_id", UUID.class), rs.getString("channel"), rs.getString("title"), rs.getString("body"),
                instant(rs.getObject("scheduled_at", OffsetDateTime.class))) : null, tenantId, messageNo);
    }

    private static boolean same(ExistingMessage existing, UUID templateId, CreateCommand command,
                                NotificationTemplateRenderer.Rendered rendered, Instant scheduledAt) {
        return Objects.equals(existing.templateId(), templateId)
                && Objects.equals(existing.recipientType(), command.recipientType().trim())
    ...
    static String messageNo(UUID tenantId, String requestKey) {
        try {
            byte[] digest = MessageDigest.getInstance("SHA-256").digest((tenantId + "|" + requestKey).getBytes(StandardCharsets.UTF_8));
    ...
    private static void validate(CreateCommand command) {
        if (command == null || command.tenantId() == null) throw new IllegalArgumentException("notification tenant is required");
    ...
    public record CreateCommand(UUID tenantId, UUID actorId, String requestKey, String templateCode, String channel,
                                String recipientType, UUID recipientId, Map<String, String> variables, Instant scheduledAt) {}
    private record Template(UUID id, String channel, String titleTemplate, String bodyTemplate, String variablesSchema) {}
    private record ExistingMessage(UUID id, UUID templateId, String recipientType, UUID recipientId, String channel,
