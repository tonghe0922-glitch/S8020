...
public class JdbcSignatureEnvelopeRepository implements SignatureEnvelopeService.Repository {
    ...
    public void recordEvidence(
            UUID tenantId, UUID envelopeId, SignatureEnvelopeService.CallbackEvidence evidence, UUID actorId) {
        ...
        for (SignatureEnvelopeService.PartyEvidence party : evidence.partyEvidence()) {
            ...
            if (updated != 1) {
                throw new ProcessRejectedException(
                        "signature callback party evidence does not match exactly one party");
            }
        }
    }
    ...
    private static SignatureEnvelopeService.Envelope map(ResultSet rs) throws SQLException {
        return new SignatureEnvelopeService.Envelope(
                ...
                rs.getString("envelope_no"),
                rs.getString("status"),
                rs.getInt("version_no"),
                rs.getString("document_hash"),
                rs.getString("template_version"),
                rs.getString("signing_order"),
                instant(rs, "sign_deadline_at"),
                rs.getString("sign_status"),
                rs.getObject("completed_file_id", UUID.class),
...
@Component
class JdbcFileEvidenceCapability implements SignatureEnvelopeService.FileEvidenceCapability {
    private final JdbcTemplate jdbc;
    private final String safeStatus;

    JdbcFileEvidenceCapability(
