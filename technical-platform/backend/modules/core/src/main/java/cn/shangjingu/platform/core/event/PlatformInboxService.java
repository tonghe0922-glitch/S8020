...
@Component
public class PlatformInboxService {
    private final JdbcTemplate jdbc;

    public PlatformInboxService(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }

    public boolean claim(UUID tenantId, String consumerName, String eventKey) {
        validate(tenantId, consumerName, eventKey);
        requireTransaction();
        ...
        List<String> results = jdbc.query(
                ...
                """,
                (rs, rowNum) -> rs.getString("result_code"),
                tenantId,
                consumer,
                key);
        if (!results.isEmpty()) {
            if (results.size() != 1 || !"SUCCESS".equals(results.getFirst()))
                throw new IllegalStateException("Inbox contains ambiguous or incomplete persisted processing evidence");
            return false;
        }
    ...
    private static void validate(UUID tenantId, String consumerName, String eventKey) {
        if (tenantId == null) throw new IllegalArgumentException("tenantId is required");
        requireText(consumerName, 128, "consumerName");
        requireText(eventKey, 128, "eventKey");
    }

    private static void requireTransaction() {
        if (!TransactionSynchronizationManager.isActualTransactionActive())
            throw new IllegalStateException("Inbox claim requires an active transaction");
    }
