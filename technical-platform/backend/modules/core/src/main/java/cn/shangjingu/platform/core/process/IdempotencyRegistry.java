...
@Component
public final class IdempotencyRegistry {
    ...
    public IdempotencyClaim claim(
        ...
        jdbc.update("delete from core.idempotency_record where tenant_id=? and idempotency_key=? and expire_at <= now()",
        ...
        try {
            jdbc.update("""
                    ...
                    """,
                    UUID.randomUUID(), tenantId, actorId, actorId, idempotencyKey, requestHash,
                    resourceType, proposedResourceId, ttl.toMillis());
    ...
    private IdempotencyClaim existing(UUID tenantId, String key, String requestHash, String resourceType) {
        return jdbc.query("""
                ...
                """, rs -> {
                    ...
                    if (!requestHash.equals(rs.getString("request_hash")) || !resourceType.equals(rs.getString("resource_type"))) {
