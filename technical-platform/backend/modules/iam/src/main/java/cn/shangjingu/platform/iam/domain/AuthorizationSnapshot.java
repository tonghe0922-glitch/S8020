...
public record AuthorizationSnapshot(Set<String> permissions, List<AuthorizationGrant> grants) {
    public static AuthorizationSnapshot from(List<AuthorizationGrant> raw) {
        List<AuthorizationGrant> usable = raw.stream().filter(AuthorizationGrant::unconditional).toList();
        Set<String> permissions = usable.stream().map(AuthorizationGrant::permissionCode).collect(java.util.stream.Collectors.toUnmodifiableSet());
