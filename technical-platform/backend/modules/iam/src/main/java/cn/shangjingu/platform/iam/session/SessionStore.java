...
public interface SessionStore {
    enum RotationOutcome { ROTATED, MISSING, REPLAYED }
    record StoredSession(UUID familyId, SessionContext context, String accessDigest, String refreshDigest,
    ...
    RotationOutcome rotate(StoredSession current,String presentedRefreshDigest,StoredSession replacement,
                           Duration accessTtl,Duration refreshTtl);
    void revoke(StoredSession session);
    default List<StoredSession> listByUser(UUID tenantId, UUID userId) { return List.of(); }
}
