...
public class WorkflowSystemActionService {
    ...
    public WorkflowRuntimeService.Result act(SystemActionCommand command) {
        ...
        WorkflowIdempotency.Claim claim = idempotency.claim(
                ...
                requestHash,
                "WORKFLOW_SYSTEM_ACTION",
                proposedActionId);
        if (claim.existing()) {
            WorkflowRuntimeService.ActionLog action = repository
                    .findAction(command.tenantId(), claim.resourceId())
                    .orElseThrow(() -> WorkflowException.conflict(
                            "system-action idempotency points to a missing workflow action"));
            WorkflowRuntimeService.Instance existing = repository
                    .findInstance(command.tenantId(), action.instanceId())
                    .orElseThrow(
                            () -> WorkflowException.conflict("system workflow action points to a missing instance"));
            WorkflowRuntimeService.Task task = repository
        ...
        WorkflowRuntimeService.Instance instance = repository
        ...
        if (!command.expectedNodeCode().trim().equals(instance.currentNodeCode())) {
            throw new WorkflowException(
                    WorkflowException.Code.STALE_VERSION, "workflow current node changed before system action");
        }
        WorkflowRuntimeService.RuntimeNode current = repository
        ...
        if (WorkflowRuntimeService.START_NODE.equalsIgnoreCase(current.nodeType())
            ...
            throw new WorkflowException(
                    WorkflowException.Code.ILLEGAL_ACTION, "system actions require a non-terminal workflow task node");
        }

        WorkflowRuntimeService.Task visibleTask = repository
                .findCurrentTask(command.tenantId(), instance.id(), instance.currentNodeCode())
                .orElseThrow(() -> new WorkflowException(
                        WorkflowException.Code.STALE_VERSION, "current workflow task is missing"));
        WorkflowRuntimeService.Task currentTask = repository
                .lockTask(command.tenantId(), visibleTask.id())
        ...
        if (!WorkflowRuntimeService.PENDING.equals(currentTask.status())
            ...
            throw new WorkflowException(
                    WorkflowException.Code.STALE_VERSION, "workflow task changed before system action");
        }

        List<WorkflowRuntimeService.RuntimeTransition> matching = new ArrayList<>();
        for (WorkflowRuntimeService.RuntimeTransition transition :
                repository.listTransitions(command.tenantId(), instance.versionId(), current.nodeCode(), actionCode)) {
        ...
        if (matching.isEmpty()) {
            throw new WorkflowException(
                    WorkflowException.Code.ILLEGAL_ACTION, "system action is not allowed from current workflow node");
        }
        if (matching.size() != 1) {
            throw new WorkflowException(
                    WorkflowException.Code.INVALID_DEFINITION,
        ...
        WorkflowRuntimeService.RuntimeNode target = repository
        ...
        int completed =
                repository.completeTask(command.tenantId(), currentTask.id(), null, actionCode, command.reason(), now);
        if (completed != 1) {
            throw new WorkflowException(
                    WorkflowException.Code.STALE_VERSION, "workflow task changed concurrently during system action");
        }
    ...
    private String hash(Object value) {
        try {
            ...
            throw new WorkflowException(
                    WorkflowException.Code.INVALID_ARGUMENT, "cannot hash system workflow request", failure);
        }
    }
