...
public class GuardedOvertimeRepository implements OvertimeService.Repository {
    ...
    public GuardedOvertimeRepository(JdbcOvertimeRepository delegate, JdbcTemplate jdbc) {
    ...
    @Override
    public boolean hasLeaveConflict(UUID tenantId, UUID employeeId, Instant start, Instant end) {
    ...
    public int markNecessity(UUID tenantId, UUID id, UUID actor) {
        return required(delegate.markNecessity(tenantId, id, actor), "necessity validation fact");
    }

    @Override
    public int markDecision(UUID tenantId, UUID id, String decision, UUID actor) {
        return required(delegate.markDecision(tenantId, id, decision, actor), "supervisor decision fact");
    }

    @Override
    public int recordActual(
            UUID tenantId, UUID id, Instant start, Instant end, BigDecimal hours, String summary, UUID actor) {
        return required(delegate.recordActual(tenantId, id, start, end, hours, summary, actor), "actual labor fact");
    }

    @Override
    public int acceptResult(UUID tenantId, UUID id, String result, UUID actor) {
        return required(delegate.acceptResult(tenantId, id, result, actor), "result acceptance fact");
    }
    ...
    public int setCompensationPlan(
            UUID tenantId, UUID id, String plan, BigDecimal wage, String quotaAccount, BigDecimal timeOff, UUID actor) {
        return required(
                delegate.setCompensationPlan(tenantId, id, plan, wage, quotaAccount, timeOff, actor),
                "compensation plan fact");
    }

    @Override
    public int ackPayroll(UUID tenantId, UUID id, String reference, UUID actor) {
        return required(delegate.ackPayroll(tenantId, id, reference, actor), "payroll receipt fact");
    }
    ...
    public void appendTimeOffLedger(
            UUID tenantId, UUID id, String entryType, BigDecimal amount, String note, UUID actor) {
        if (!"GRANT".equals(entryType)) {
            throw new ProcessRejectedException("P009 time-off ledger entry type is invalid");
        }
        if (amount == null || amount.signum() <= 0) {
            throw new ProcessRejectedException("P009 time-off ledger amount must be positive");
        }
