...
public class JdbcOvertimeRepository implements OvertimeService.Repository {
    ...
    public Optional<FormRef> form(UUID t) {
        return jdbc
                .query(
                        ...
                        (rs, n) -> new FormRef(rs.getObject(1, UUID.class), rs.getInt(2)),
                        t)
                .stream()
                .findFirst();
    }
    ...
    public int bindAndMove(UUID t, UUID id, int v, UUID wf, String status, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set workflow_instance_id=?,status=?,version_no=version_no+1,updated_by=?,updated_at=now() where tenant_id=? and id=? and version_no=? and not is_deleted",
                wf,
                status,
                actor,
                t,
    ...
    public int moveStatus(UUID t, UUID id, int v, String status, Instant closed, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set status=?,closed_at=coalesce(?,closed_at),version_no=version_no+1,updated_by=?,updated_at=now() where tenant_id=? and id=? and version_no=? and not is_deleted",
                status,
                ts(closed),
                actor,
                t,
    ...
    public int markNecessity(UUID t, UUID id, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set necessity_checked_at=coalesce(necessity_checked_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and supervisor_decision is null and not is_deleted",
                actor,
                t,
                id);
    }
    ...
    public int acceptResult(UUID t, UUID id, String result, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set result_summary=?,result_accepted_at=coalesce(result_accepted_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and actual_fact_recorded_at is not null and not is_deleted",
                result,
                actor,
                t,
                id);
    ...
    public int markHrReviewed(UUID t, UUID id, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set hr_reviewed_at=coalesce(hr_reviewed_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and result_accepted_at is not null and not is_deleted",
                actor,
                t,
                id);
    }
    ...
    public int setCompensationPlan(
        ...
        return jdbc.update(
                "update attendance.overtime_request set compensation_plan=?,actual_amount=?,quota_account_id=?,quota_amount=?,compensation_planned_at=coalesce(compensation_planned_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and hr_reviewed_at is not null and compensation_plan is null and not is_deleted",
                plan,
                wage,
                account,
                timeOff,
    ...
    public int ackPayroll(UUID t, UUID id, String ref, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set payroll_reference=?,payroll_receipt_at=coalesce(payroll_receipt_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and compensation_planned_at is not null and not is_deleted",
                ref,
                actor,
                t,
                id);
    ...
    public int archive(UUID t, UUID id, UUID actor) {
        return jdbc.update(
                "update attendance.overtime_request set archived_at=coalesce(archived_at,now()),updated_by=?,updated_at=now() where tenant_id=? and id=? and payroll_receipt_at is not null and not is_deleted",
                actor,
                t,
                id);
    }
    ...
    public void appendTimeOffLedger(UUID t, UUID id, String type, BigDecimal amount, String note, UUID actor) {
        jdbc.queryForObject(
                "select id from attendance.overtime_request where tenant_id=? and id=? and not is_deleted for update",
                UUID.class,
                t,
                id);
        Integer seq = jdbc.queryForObject(
                "select coalesce(max(item_seq),0)+1 from attendance.overtime_request_item where tenant_id=? and master_id=? and field_code='TIME_OFF_LEDGER' and not is_deleted",
                Integer.class,
                t,
                id);
        jdbc.update(
    ...
    public Optional<OvertimeRecord> find(UUID t, UUID id) {
        return jdbc
                .query(select("where r.tenant_id=? and r.id=? and not r.is_deleted"), (rs, n) -> map(rs), t, id)
                .stream()
                .findFirst();
    }
    ...
    public List<OvertimeRecord> list(UUID t) {
        return jdbc.query(
                select("where r.tenant_id=? and not r.is_deleted order by r.created_at desc,r.id desc"),
                (rs, n) -> map(rs),
                t);
    }
    ...
    private OvertimeRecord map(ResultSet rs) throws SQLException {
        return new OvertimeRecord(
                ...
                rs.getBoolean("emergency_fact"),
                instant(rs, "necessity_checked_at"),
                rs.getString("supervisor_decision"),
                instant(rs, "supervisor_approved_at"),
                instant(rs, "supervisor_rejected_at"),
                instant(rs, "actual_start_at"),
                instant(rs, "actual_end_at"),
                rs.getBigDecimal("actual_duration_hours"),
                rs.getString("actual_attendance_summary"),
                instant(rs, "actual_fact_recorded_at"),
                rs.getString("result_summary"),
                instant(rs, "result_accepted_at"),
                instant(rs, "hr_reviewed_at"),
                rs.getString("compensation_plan"),
                rs.getBigDecimal("actual_amount"),
                rs.getString("quota_account_id"),
                rs.getBigDecimal("quota_amount"),
                instant(rs, "compensation_planned_at"),
                rs.getString("payroll_reference"),
                instant(rs, "payroll_receipt_at"),
