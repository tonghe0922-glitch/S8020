...
public class P005NoticeController {
    ...
    public NoticeView publish(
        ...
        NoticeReceiptService.NoticeAggregate published =
                notices.publish(context(principal), idempotencyKey, hash(command), command);
    ...
    public NoticeView markRead(
        ...
        require(authorization.authorizeData(
                principal.context(),
                RECEIPT,
                target(recipient, principal.context().tenantId())));
        audit.recordOperation(principal.context(), "P005_READ_ATTEMPT", "collaboration.notice", id);
        NoticeReceiptService.NoticeAggregate result =
                notices.markRead(context(principal), id, idempotencyKey, hash(command), command);
    ...
    public NoticeView confirm(
        ...
        NoticeReceiptService.NoticeAggregate result =
                notices.confirm(context(principal), id, idempotencyKey, hash(command), command);
    ...
    public NoticeView understanding(
        ...
        NoticeReceiptService.NoticeAggregate result =
                notices.understanding(context(principal), id, idempotencyKey, hash(command), command);
    ...
    public NoticeView execution(
        ...
        NoticeReceiptService.NoticeAggregate result =
                notices.execute(context(principal), id, idempotencyKey, hash(command), command);
    ...
    private NoticeView viewOrNull(
            ...
            NoticeReceiptService.NoticeAggregate aggregate,
            boolean canRead,
            boolean canManage,
            boolean canMonitor) {
        if (canManage
                && authorization
                        .authorizeData(principal.context(), MANAGE, target(aggregate.notice()))
                        .allowed()) {
            return view(principal, aggregate, MANAGE);
        }
        NoticeReceiptService.Recipient own = aggregate.recipients().stream()
                .filter(r -> principal.context().employeeId().equals(r.employeeId()))
                .findFirst()
                .orElse(null);
        if (canRead
                && own != null
                && authorization
                        .authorizeData(
                                principal.context(),
                                READ,
                                target(own, aggregate.notice().tenantId()))
                        .allowed()) {
            return recipientView(aggregate, principal.context().employeeId());
        }
        if (canMonitor
                && authorization
                        .authorizeData(principal.context(), MONITOR, target(aggregate.notice()))
                        .allowed()) {
    ...
    private NoticeView view(
            SessionPrincipal principal, NoticeReceiptService.NoticeAggregate aggregate, String permission) {
    ...
    private static NoticeView fullView(NoticeReceiptService.NoticeAggregate aggregate) {
        return new NoticeView(
                ...
                aggregate.recipientCount(),
                aggregate.deliveredCount(),
                aggregate.readCount(),
                aggregate.confirmedCount(),
                aggregate.understandingPassedCount(),
