...
public class P006MeetingController {
    ...
    @PostMapping
    public MeetingService.MeetingAggregate create(
    ...
    public List<MeetingService.MeetingAggregate> list(@AuthenticationPrincipal SessionPrincipal principal) {
        boolean read = allowed(principal, READ),
                manage = allowed(principal, MANAGE),
                monitor = allowed(principal, MONITOR);
        ...
        List<MeetingService.MeetingAggregate> result = meetings.list(context(principal)).stream()
                .map(a -> project(principal, a, read, manage, monitor))
    ...
    @GetMapping("/{id}")
    public MeetingService.MeetingAggregate get(
    ...
    public MeetingService.MeetingAggregate action(
            @AuthenticationPrincipal SessionPrincipal principal,
    ...
    private MeetingService.MeetingAggregate project(
            SessionPrincipal principal,
            MeetingService.MeetingAggregate a,
            boolean read,
            boolean manage,
            boolean monitor) {
        ...
        if (manage
                && authorization
                        ...
                        .allowed()) return a;
        if (read
                && a.employeeVisible(principal.context().employeeId())
                && authorization
                        .authorizeData(principal.context(), READ, participantTarget(a.meeting(), principal))
                        .allowed()) return a;
        if (monitor
                && authorization
                        .authorizeData(principal.context(), MONITOR, target)
                        .allowed()) return a.metadataOnly();
        return null;
    }
    ...
    private boolean allowed(SessionPrincipal p, String permission) {
        return authorization.authorizeAction(p.context(), permission).allowed();
    }
    ...
    private static AccessDeniedException denied(String reason) {
        return new AccessDeniedException("P006 authorization denied: " + reason);
    }

    private static String safeAction(String code) {
        if (code == null) return "INVALID";
        String v = code.trim().toUpperCase();
        return v.matches("[A-Z0-9_]{1,32}") ? v : "INVALID";
    }
    ...
    private String hash(Object value) {
        try {
            ...
            throw new IllegalArgumentException("P006 request cannot be hashed", ex);
        }
    }
}
