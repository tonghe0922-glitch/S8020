...
public class PlatformWebhookRuntimeConfiguration {
    ...
    WebhookIngressService platformWebhookIngressService(JdbcTemplate jdbc, TransactionalOutboxService outbox) {
        return new WebhookIngressService(jdbc, outbox);
    }

    @Bean
    @ConditionalOnMissingBean(WebhookSignatureVerifier.class)
    WebhookSignatureVerifier failClosedWebhookSignatureVerifier() {
