...
public class PlatformDocumentRuntimeConfiguration {
    ...
    FileDownloadAuthorizationTargetResolver fileDownloadAuthorizationTargetResolver() {
        return file -> Optional.empty();
    }

    @Bean
    @ConditionalOnMissingBean
    FileDownloadGuard platformFileDownloadGuard(
    ...
    @Bean
    @ConditionalOnMissingBean
    MinioClient platformMinioClient(
            @Value("${platform.file.minio.endpoint}") String endpoint,
        ...
        if (endpoint == null
                ...
                || secretKey == null
                || secretKey.isBlank())
            throw new IllegalStateException("platform.file MinIO credentials must be explicitly configured");
    ...
    @Bean
    @ConditionalOnMissingBean
    FileObjectStorage platformFileObjectStorage(MinioClient client) {
        return new MinioFileObjectStorage(client);
    }
