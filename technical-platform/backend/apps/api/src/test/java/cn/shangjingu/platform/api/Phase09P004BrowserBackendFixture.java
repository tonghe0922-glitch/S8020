...
public final class Phase09P004BrowserBackendFixture {
    ...
    private static void seed(PostgreSQLContainer<?> postgres, String applicantLogin, String password) throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(
            ...
            s.execute("INSERT INTO org.organization(id,tenant_id,org_code,org_name,org_type,path,status) VALUES "
                    + "('" + CENTER_A + "','" + TENANT
                    + "','P004_CENTER_A','P004 Center A','CENTER','p004_center_a'::ltree,'ACTIVE'),"
                    + "('" + CENTER_B + "','" + TENANT
                    + "','P004_CENTER_B','P004 Center B','CENTER','p004_center_b'::ltree,'ACTIVE')");
            s.execute("INSERT INTO org.position(id,tenant_id,position_code,position_name,org_id,status) VALUES "
                    + "('" + POSITION_A + "','" + TENANT + "','P004_POS_A','P004 Position A','" + CENTER_A
                    + "','ACTIVE'),"
                    + "('" + POSITION_B + "','" + TENANT + "','P004_POS_B','P004 Position B','" + CENTER_B
                    + "','ACTIVE')");
            s.execute(
                    "INSERT INTO org.employee(id,tenant_id,employee_no,person_name,employment_status,hire_date,primary_org_id,primary_position_id) VALUES "
                            ...
                            + employee(OUT_EMPLOYEE, "P004-E005", "P004 Cross Center", CENTER_B, POSITION_B));
            s.execute(
                    "INSERT INTO org.employee_position(id,tenant_id,employee_id,position_id,org_id,is_primary,effective_start_date,status) VALUES "
                            ...
                            + appointment(ACTOR1_APPOINTMENT, ACTOR1_EMPLOYEE, POSITION_A, CENTER_A) + ","
                            + appointment(ACTOR2_APPOINTMENT, ACTOR2_EMPLOYEE, POSITION_A, CENTER_A) + ","
            ...
            s.execute("INSERT INTO iam.user_account(id,tenant_id,login_name,password_hash,status,mfa_level) VALUES "
                    + account(APPLICANT_USER, applicantLogin, hash) + "," + account(ACTOR1_USER, ACTOR1_LOGIN, hash)
                    + "," + account(ACTOR2_USER, ACTOR2_LOGIN, hash) + "," + account(TECH_USER, TECH_LOGIN, hash) + ","
            ...
            s.execute("INSERT INTO iam.role(id,tenant_id,role_code,role_name,role_type,data_scope_code,enabled) VALUES "
                    + role(APPLICANT_ROLE, "P004_APPLICANT", "P004 Applicant", "P004_SELF") + ","
                    + role(ACTOR_ROLE, "P004_ACTOR", "P004 Actor", "P004_CENTER") + ","
                    + role(TECH_ROLE, "P004_TECH", "P004 Tech", "P004_CENTER") + ","
                    + role(OUT_ROLE, "P004_OUT", "P004 Out", "P004_CENTER"));
            s.execute(
                    "INSERT INTO iam.permission(id,tenant_id,permission_code,permission_name,resource_type,action_code,risk_level) VALUES "
                            ...
                            + ","
                            + permission(
                                    P004_ACT, "p004.request.act", "Act P004", "P004_GENERIC_REQUEST", "ACT", "HIGH"));
            s.execute("INSERT INTO iam.role_permission(tenant_id,role_id,permission_id) VALUES "
                    ...
                    + rp(APPLICANT_ROLE, P004_SUBMIT) + "," + rp(APPLICANT_ROLE, P004_READ) + ","
                    + rp(ACTOR_ROLE, SESSION_READ) + "," + rp(ACTOR_ROLE, SESSION_LOGOUT) + ","
                    + rp(ACTOR_ROLE, P004_READ) + "," + rp(ACTOR_ROLE, P004_ACT) + "," + rp(TECH_ROLE, SESSION_READ)
                    + "," + rp(TECH_ROLE, SESSION_LOGOUT) + "," + rp(TECH_ROLE, P004_READ) + ","
                    + rp(OUT_ROLE, SESSION_READ) + "," + rp(OUT_ROLE, SESSION_LOGOUT) + "," + rp(OUT_ROLE, P004_READ)
                    + "," + rp(OUT_ROLE, P004_ACT));
            s.execute(
                    "INSERT INTO iam.user_role(tenant_id,user_id,identity_id,role_id,effective_start_at,grant_source) VALUES "
                            ...
                            + ur(ACTOR1_USER, ACTOR1_IDENTITY, ACTOR_ROLE) + ","
                            + ur(ACTOR2_USER, ACTOR2_IDENTITY, ACTOR_ROLE) + ","
                            + ur(TECH_USER, TECH_IDENTITY, TECH_ROLE) + "," + ur(OUT_USER, OUT_IDENTITY, OUT_ROLE));
        }
    }

    private static String employee(UUID id, String no, String name, UUID org, UUID pos) {
        return "('" + id + "','" + TENANT + "','" + no + "','" + name + "','ACTIVE',current_date-30,'" + org + "','"
                + pos + "')";
    }

    private static String appointment(UUID id, UUID employee, UUID pos, UUID org) {
        return "('" + id + "','" + TENANT + "','" + employee + "','" + pos + "','" + org
    ...
    private static String identity(UUID id, UUID user, UUID employee, String name, UUID org, UUID pos) {
        return "('" + id + "','" + TENANT + "','" + user + "','" + employee + "','EMPLOYEE','" + name + "','" + org
                + "','" + pos + "',true,now()-interval '1 day')";
    }
    ...
    private static void writeRuntimeFacts(PostgreSQLContainer<?> postgres, GenericContainer<?> redis) throws Exception {
        Path output =
                findRepoRoot().resolve("technical-platform/backend/apps/api/target/phase09-p004-fixture-runtime.json");
