...
class PlatformFileDownloadGuardTest {
    ...
    private final AuthorizationTarget target = new AuthorizationTarget(
    ...
    void permissionDataScopeMfa2AndCriticalAuditAreAllRequiredBeforeDownload() {
        when(authorization.authorizeAction(subject, PlatformFileDownloadGuard.PERMISSION))
                .thenReturn(AuthorizationDecision.allow(PlatformFileDownloadGuard.PERMISSION, "OWNER"));
        when(authorization.authorizeData(subject, PlatformFileDownloadGuard.PERMISSION, target))
                .thenReturn(AuthorizationDecision.allow(PlatformFileDownloadGuard.PERMISSION, "OWNER"));
        PlatformFileDownloadGuard guard =
                new PlatformFileDownloadGuard(authorization, stepUp, audit, ignored -> Optional.of(target));
        try (FileDownloadAuthorizationContext.Scope ignored =
                FileDownloadAuthorizationContext.open(subject, "ticket-661")) {
            guard.requireAllowed(file);
        }
    ...
    void missingAuthoritativeDataTargetFailsClosedBeforeStepUpOrAudit() {
        when(authorization.authorizeAction(subject, PlatformFileDownloadGuard.PERMISSION))
                .thenReturn(AuthorizationDecision.allow(PlatformFileDownloadGuard.PERMISSION, "OWNER"));
        PlatformFileDownloadGuard guard =
                new PlatformFileDownloadGuard(authorization, stepUp, audit, ignored -> Optional.empty());
        try (FileDownloadAuthorizationContext.Scope ignored =
                FileDownloadAuthorizationContext.open(subject, "ticket")) {
    ...
    void immutableAuditFailureEscapesAndBlocksSignedUrlCreation() {
        ...
        when(authorization.authorizeData(subject, PlatformFileDownloadGuard.PERMISSION, target))
                .thenReturn(AuthorizationDecision.allow(PlatformFileDownloadGuard.PERMISSION, "OWNER"));
        doThrow(new SecurityAuditUnavailableException(new IllegalStateException("audit offline")))
                .when(audit)
                .recordSensitiveAccess(
                        subject, "document.file_object", file.id(), "[\"content\"]", "SIGNED_FILE_DOWNLOAD");
        PlatformFileDownloadGuard guard =
    ...
    private static FileObjectService.FileObject file(UUID tenant) {
        return new FileObjectService.FileObject(
                UUID.randomUUID(),
                tenant,
                "tenant/object",
                "secret.pdf",
