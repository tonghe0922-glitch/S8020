...
public final class Phase09P002BrowserBackendFixture {
    ...
    private static void seedSyntheticFacts(PostgreSQLContainer<?> postgres, String applicantLogin, String password)
            throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(
            ...
            statement.execute(
                    "INSERT INTO org.organization(id,tenant_id,org_code,org_name,org_type,path,status) VALUES "
                            + "('" + CENTER_A + "','" + TENANT
                            + "','PHASE09_P002_CENTER_A','Synthetic P002 Center A','CENTER','phase09_p002_center_a'::ltree,'ACTIVE'),"
                            + "('" + CENTER_B + "','" + TENANT
                            + "','PHASE09_P002_CENTER_B','Synthetic P002 Center B','CENTER','phase09_p002_center_b'::ltree,'ACTIVE')");
            statement.execute("INSERT INTO org.position(id,tenant_id,position_code,position_name,org_id,status) VALUES "
                    + "('" + POSITION_A + "','" + TENANT + "','PHASE09_P002_POS_A','Synthetic P002 Position A','"
                    + CENTER_A + "','ACTIVE'),"
                    + "('" + POSITION_B + "','" + TENANT + "','PHASE09_P002_POS_B','Synthetic P002 Position B','"
                    + CENTER_B + "','ACTIVE')");
            statement.execute(
            ...
            statement.execute(
                    "INSERT INTO iam.user_identity(id,tenant_id,user_id,employee_id,identity_type,identity_name,org_id,position_id,is_primary,effective_start_at) VALUES "
                            ...
                            + ","
                            + identity(
                                    TECH_IDENTITY, TECH_USER, TECH_EMPLOYEE, "P002 Tech Identity", CENTER_A, POSITION_A)
                            + ","
                            + identity(
                                    OUT_IDENTITY, OUT_USER, OUT_EMPLOYEE, "P002 Out Identity", CENTER_B, POSITION_B));
            statement.execute(
                    "INSERT INTO iam.data_scope_rule(tenant_id,scope_code,scope_name,rule_expr,enabled) VALUES "
                            + "('" + TENANT
                            + "','PHASE09_P002_SELF','P002 Self Scope','{\"scope\":\"SELF\"}'::jsonb,true),"
                            + "('" + TENANT
                            + "','PHASE09_P002_CENTER','P002 Center Scope','{\"scope\":\"CENTER\"}'::jsonb,true)");
            statement.execute(
                    "INSERT INTO iam.role(id,tenant_id,role_code,role_name,role_type,data_scope_code,enabled) VALUES "
                            ...
                            + role(
                                    REQUESTED_HIGH_ROLE,
                                    "PHASE09_P002_HIGH_TARGET",
                                    "P002 High Risk Requested Role",
                                    "PHASE09_P002_SELF"));
            statement.execute(
                    "INSERT INTO iam.permission(id,tenant_id,permission_code,permission_name,resource_type,action_code,risk_level) VALUES "
                            + sessionPermission(
                                    PERMISSION_SESSION_READ, "platform.session.read", "Read current session", "READ")
                            + ","
                            + sessionPermission(
                                    PERMISSION_SESSION_LOGOUT,
                                    "platform.session.logout",
                                    "Logout current session",
                                    "LOGOUT")
                            + ","
                            + permission(
                                    PERMISSION_SUBMIT, "p002.request.submit", "Submit P002 request", "SUBMIT", "NORMAL")
                            + ","
                            + permission(PERMISSION_READ, "p002.request.read", "Read P002 request", "READ", "NORMAL")
                            + ","
                            + permission(
                                    PERMISSION_REVIEW, "p002.request.review", "Review P002 request", "REVIEW", "HIGH")
                            + ","
                            + permission(
                                    PERMISSION_EXECUTE, "p002.request.execute", "Execute P002 grant", "EXECUTE", "HIGH")
                            + ","
                            + permission(
                                    PERMISSION_REVOKE, "p002.request.revoke", "Revoke P002 grant", "REVOKE", "HIGH")
                            + ","
                            + permission(
                                    PERMISSION_HIGH,
                                    "phase09.p002.synthetic.high",
                                    "Synthetic high-risk target permission",
                                    "USE",
                                    "HIGH"));
            statement.execute("INSERT INTO iam.role_permission(tenant_id,role_id,permission_id) VALUES "
                    + rolePermission(APPLICANT_ROLE, PERMISSION_SESSION_READ) + ","
                    + rolePermission(APPLICANT_ROLE, PERMISSION_SESSION_LOGOUT) + ","
                    + rolePermission(APPLICANT_ROLE, PERMISSION_SUBMIT) + ","
                    + rolePermission(APPLICANT_ROLE, PERMISSION_READ) + ","
                    + rolePermission(REVIEW_ROLE, PERMISSION_SESSION_READ) + ","
                    + rolePermission(REVIEW_ROLE, PERMISSION_SESSION_LOGOUT) + ","
                    + rolePermission(REVIEW_ROLE, PERMISSION_READ) + ","
                    + rolePermission(REVIEW_ROLE, PERMISSION_REVIEW) + ","
                    + rolePermission(TECH_ROLE, PERMISSION_SESSION_READ) + ","
                    + rolePermission(TECH_ROLE, PERMISSION_SESSION_LOGOUT) + ","
                    + rolePermission(TECH_ROLE, PERMISSION_READ) + "," + rolePermission(TECH_ROLE, PERMISSION_EXECUTE)
                    + "," + rolePermission(TECH_ROLE, PERMISSION_REVOKE) + ","
                    + rolePermission(OUT_ROLE, PERMISSION_SESSION_READ) + ","
                    + rolePermission(OUT_ROLE, PERMISSION_SESSION_LOGOUT) + ","
                    + rolePermission(OUT_ROLE, PERMISSION_READ) + ","
                    + rolePermission(REQUESTED_HIGH_ROLE, PERMISSION_HIGH));
            statement.execute(
    ...
    private static String permission(UUID id, String code, String name, String action, String risk) {
        return "('" + id + "','" + TENANT + "','" + code + "','" + name + "','P002_PERMISSION_REQUEST','" + action
                + "','" + risk + "')";
    }
    ...
    private static void writeRuntimeFacts(PostgreSQLContainer<?> postgres, GenericContainer<?> redis) throws Exception {
        Path output =
                findRepoRoot().resolve("technical-platform/backend/apps/api/target/phase09-p002-fixture-runtime.json");
        String json = "{\n"
