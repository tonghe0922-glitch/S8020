...
public final class Phase09P003BrowserBackendFixture {
    ...
    private static ConfigurableApplicationContext startApi(PostgreSQLContainer<?> postgres, GenericContainer<?> redis, String profileKey) {
        SpringApplication application = new SpringApplication(ApiApplication.class);
        return application.run(
                "--server.port=18083", "--spring.flyway.enabled=false",
    ...
    private static void prepareDatabases(PostgreSQLContainer<?> postgres, String tenantCode, String applicantLogin, String password) throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(postgres.getJdbcUrl(), postgres.getUsername(), postgres.getPassword()); Statement s = c.createStatement()) {
            ...
            s.execute("CREATE DATABASE sjg_oms"); s.execute("CREATE DATABASE sjg_audit");
        }
        Flyway.configure().dataSource(jdbcUrl(postgres, "sjg_oms"), postgres.getUsername(), postgres.getPassword())
                ...
                .placeholders(Map.of("sjg_tenant_id", TENANT.toString(), "sjg_tenant_code", tenantCode,
                        "sjg_tenant_name", "PHASE09 P003 Browser Tenant"))
    ...
    private static void seed(PostgreSQLContainer<?> postgres, String applicantLogin, String password) throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(jdbcUrl(postgres, "sjg_oms"), postgres.getUsername(), postgres.getPassword()); Statement s = c.createStatement()) {
            s.execute("INSERT INTO org.organization(id,tenant_id,org_code,org_name,org_type,path,status) VALUES "
                    + "('" + CENTER_A + "','" + TENANT + "','P003_CENTER_A','P003 Center A','CENTER','p003_center_a'::ltree,'ACTIVE'),"
                    + "('" + CENTER_B + "','" + TENANT + "','P003_CENTER_B','P003 Center B','CENTER','p003_center_b'::ltree,'ACTIVE')");
            s.execute("INSERT INTO org.position(id,tenant_id,position_code,position_name,org_id,status) VALUES "
                    + "('" + POSITION_A + "','" + TENANT + "','P003_POS_A','P003 Position A','" + CENTER_A + "','ACTIVE'),"
                    + "('" + POSITION_B + "','" + TENANT + "','P003_POS_B','P003 Position B','" + CENTER_B + "','ACTIVE')");
            ...
            s.execute("INSERT INTO org.employee_position(id,tenant_id,employee_id,position_id,org_id,is_primary,effective_start_date,status) VALUES "
                    + appointment(APPLICANT_APPOINTMENT,APPLICANT_EMPLOYEE,POSITION_A,CENTER_A)+","+appointment(REVIEW1_APPOINTMENT,REVIEW1_EMPLOYEE,POSITION_A,CENTER_A)+","+appointment(REVIEW2_APPOINTMENT,REVIEW2_EMPLOYEE,POSITION_A,CENTER_A)+","+appointment(TECH_APPOINTMENT,TECH_EMPLOYEE,POSITION_A,CENTER_A)+","+appointment(OUT_APPOINTMENT,OUT_EMPLOYEE,POSITION_B,CENTER_B));
            s.execute("INSERT INTO iam.user_account(id,tenant_id,login_name,password_hash,status,mfa_level) VALUES "
                    + account(APPLICANT_USER,applicantLogin,hash)+","+account(REVIEW1_USER,REVIEW1_LOGIN,hash)+","+account(REVIEW2_USER,REVIEW2_LOGIN,hash)+","+account(TECH_USER,TECH_LOGIN,hash)+","+account(OUT_USER,OUT_LOGIN,hash));
            s.execute("INSERT INTO iam.user_identity(id,tenant_id,user_id,employee_id,identity_type,identity_name,org_id,position_id,is_primary,effective_start_at) VALUES "
                    + identity(APPLICANT_IDENTITY,APPLICANT_USER,APPLICANT_EMPLOYEE,"P003 Applicant",CENTER_A,POSITION_A)+","+identity(REVIEW1_IDENTITY,REVIEW1_USER,REVIEW1_EMPLOYEE,"P003 Reviewer One",CENTER_A,POSITION_A)+","+identity(REVIEW2_IDENTITY,REVIEW2_USER,REVIEW2_EMPLOYEE,"P003 Reviewer Two",CENTER_A,POSITION_A)+","+identity(TECH_IDENTITY,TECH_USER,TECH_EMPLOYEE,"P003 Tech",CENTER_A,POSITION_A)+","+identity(OUT_IDENTITY,OUT_USER,OUT_EMPLOYEE,"P003 Out",CENTER_B,POSITION_B));
            ...
            s.execute("INSERT INTO iam.role(id,tenant_id,role_code,role_name,role_type,data_scope_code,enabled) VALUES "
                    + role(APPLICANT_ROLE,"P003_APPLICANT","P003 Applicant","P003_SELF")+","+role(REVIEW_ROLE,"P003_REVIEW","P003 Review","P003_CENTER")+","+role(TECH_ROLE,"P003_TECH","P003 Tech","P003_CENTER")+","+role(OUT_ROLE,"P003_OUT","P003 Out","P003_CENTER"));
            s.execute("INSERT INTO iam.permission(id,tenant_id,permission_code,permission_name,resource_type,action_code,risk_level) VALUES "
                    + permission(SESSION_READ,"platform.session.read","Session read","SESSION","READ","NORMAL")+","+permission(SESSION_LOGOUT,"platform.session.logout","Session logout","SESSION","LOGOUT","NORMAL")+","+permission(P003_SUBMIT,"p003.change.submit","Submit P003","P003_PROFILE_CHANGE","SUBMIT","NORMAL")+","+permission(P003_READ,"p003.change.read","Read P003","P003_PROFILE_CHANGE","READ","NORMAL")+","+permission(P003_REVIEW,"p003.change.review","Review P003","P003_PROFILE_CHANGE","REVIEW","HIGH")+","+permission(P003_APPLY,"p003.change.apply","Apply P003","P003_PROFILE_CHANGE","APPLY","HIGH"));
            s.execute("INSERT INTO iam.role_permission(tenant_id,role_id,permission_id) VALUES "
                    + rp(APPLICANT_ROLE,SESSION_READ)+","+rp(APPLICANT_ROLE,SESSION_LOGOUT)+","+rp(APPLICANT_ROLE,P003_SUBMIT)+","+rp(APPLICANT_ROLE,P003_READ)+","+rp(REVIEW_ROLE,SESSION_READ)+","+rp(REVIEW_ROLE,SESSION_LOGOUT)+","+rp(REVIEW_ROLE,P003_READ)+","+rp(REVIEW_ROLE,P003_REVIEW)+","+rp(TECH_ROLE,SESSION_READ)+","+rp(TECH_ROLE,SESSION_LOGOUT)+","+rp(TECH_ROLE,P003_READ)+","+rp(TECH_ROLE,P003_APPLY)+","+rp(OUT_ROLE,SESSION_READ)+","+rp(OUT_ROLE,SESSION_LOGOUT)+","+rp(OUT_ROLE,P003_READ)+","+rp(OUT_ROLE,P003_REVIEW));
            s.execute("INSERT INTO iam.user_role(tenant_id,user_id,identity_id,role_id,effective_start_at,grant_source) VALUES "
                    + ur(APPLICANT_USER,APPLICANT_IDENTITY,APPLICANT_ROLE)+","+ur(REVIEW1_USER,REVIEW1_IDENTITY,REVIEW_ROLE)+","+ur(REVIEW2_USER,REVIEW2_IDENTITY,REVIEW_ROLE)+","+ur(TECH_USER,TECH_IDENTITY,TECH_ROLE)+","+ur(OUT_USER,OUT_IDENTITY,OUT_ROLE));
        }
    }

    private static String employee(UUID id,String no,String name,UUID center,UUID position){return "('"+id+"','"+TENANT+"','"+no+"','"+name+"','ACTIVE',current_date-30,'"+center+"','"+position+"')";}
    private static String appointment(UUID id,UUID employee,UUID position,UUID center){return "('"+id+"','"+TENANT+"','"+employee+"','"+position+"','"+center+"',true,current_date-30,'ACTIVE')";}
    private static String account(UUID id,String login,String hash){return "('"+id+"','"+TENANT+"','"+login+"','"+hash+"','ACTIVE',0)";}
    private static String identity(UUID id,UUID user,UUID employee,String name,UUID center,UUID position){return "('"+id+"','"+TENANT+"','"+user+"','"+employee+"','EMPLOYEE','"+name+"','"+center+"','"+position+"',true,now()-interval '1 day')";}
    ...
    private static void writeRuntimeFacts(PostgreSQLContainer<?> postgres, GenericContainer<?> redis) throws Exception {
        Path output = findRepoRoot().resolve("technical-platform/backend/apps/api/target/phase09-p003-fixture-runtime.json");
        ...
        Files.createDirectories(output.getParent()); Files.writeString(output,json);
    }
    private static String jdbcUrl(PostgreSQLContainer<?> postgres,String database){String url=postgres.getJdbcUrl();int q=url.indexOf('?');String suffix=q>=0?url.substring(q):"";String base=q>=0?url.substring(0,q):url;return base.substring(0,base.lastIndexOf('/')+1)+database+suffix;}
    ...
    private static String requiredEnv(String name){String value=System.getenv(name);if(value==null||value.isBlank())throw new IllegalStateException("required environment missing: "+name);return value;}
