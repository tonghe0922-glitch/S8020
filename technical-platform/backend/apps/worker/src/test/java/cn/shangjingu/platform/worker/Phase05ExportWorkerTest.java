...
class Phase05ExportWorkerTest {
    ...
    void generatorSelectionIsExactAndFailClosed() {
        SensitiveExportService.SensitiveExportGenerator generator = mock(SensitiveExportService.SensitiveExportGenerator.class);
        when(generator.exportType()).thenReturn("SYNTHETIC");
        Phase05ExportWorker worker = new Phase05ExportWorker(
                mock(TenantTransactionRunner.class), mock(JdbcTemplate.class), mock(SensitiveExportService.class),
                List.of(generator), mock(WorkerCriticalAuditService.class));
    ...
    void duplicateGeneratorsAreRejected() {
        SensitiveExportService.SensitiveExportGenerator first = mock(SensitiveExportService.SensitiveExportGenerator.class);
        SensitiveExportService.SensitiveExportGenerator second = mock(SensitiveExportService.SensitiveExportGenerator.class);
