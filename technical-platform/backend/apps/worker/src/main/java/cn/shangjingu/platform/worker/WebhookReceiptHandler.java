...
public final class WebhookReceiptHandler implements PlatformOutboxHandler {
    ...
    public WebhookReceiptHandler(WebhookProcessingService processing) {
        this.processing = processing;
    }

    @Override
    public String eventType() {
        return WebhookIngressService.EVENT_TYPE;
    }

    @Override
    public String consumerName() {
        return "platform-webhook-receipt";
    }

    @Override
    public void handle(PlatformOutboxEvent event) {
        if (!WebhookIngressService.AGGREGATE_TYPE.equals(event.aggregateType()))
            throw new IllegalStateException("webhook outbox aggregate type mismatch");
        processing.markProcessed(event.tenantId(), event.aggregateId());
    }
}
