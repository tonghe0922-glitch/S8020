...
public final class Phase05ExportWorker {
        ...
        try {
            transactions.required(tenantId, () -> {
                String status = jdbc.query("""
                        ...
                        """, rs -> rs.next() ? rs.getString(1) : null, tenantId, eventId, requestId);
                ...
                if (!"PENDING".equals(status)) throw new ProcessRejectedException("sensitive export outbox event is not pending");
                SensitiveExportService.ExportRequest request = exports.find(DatabaseSecurityContext.tenantOnly(tenantId), requestId)
                ...
                audit.record(tenantId, "P019_WORKER_GENERATE_ATTEMPT", "audit.data_export_request", requestId,
                        Map.of("event_id", eventId.toString(), "expected_version", expectedVersion, "export_type", request.exportType()));
                ...
                exports.recordGenerated(DatabaseSecurityContext.tenantOnly(tenantId), requestId, expectedVersion, result);
                if (jdbc.update("""
    ...
    SensitiveExportService.SensitiveExportGenerator resolve(String exportType) {
        List<SensitiveExportService.SensitiveExportGenerator> matches = generators.stream()
                .filter(g -> exportType.equals(g.exportType())).toList();
        if (matches.size() != 1) {
            throw new ProcessRejectedException("sensitive export generator is unavailable or ambiguous for type: " + exportType);
        }
        return matches.getFirst();
    }

    private void recordFailure(UUID tenantId, UUID eventId, UUID requestId, int expectedVersion, RuntimeException failure) {
        transactions.required(tenantId, () -> {
            ...
            if (retry != null && retry >= MAX_ATTEMPTS) {
                ...
                jdbc.update("""
                        ...
                        """, UUID.randomUUID(), requestId.toString(), expectedVersion, eventId.toString(),
