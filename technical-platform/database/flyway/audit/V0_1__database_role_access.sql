...
DO $$
BEGIN
  IF current_database() <> 'sjg_audit' THEN
    RAISE EXCEPTION 'PHASE-03 migration expected database sjg_audit, got %', current_database();
  END IF;
END
$$;
...
GRANT CONNECT ON DATABASE sjg_audit TO sjg_migration;
GRANT CONNECT ON DATABASE sjg_audit TO sjg_audit_writer;
GRANT CONNECT ON DATABASE sjg_audit TO sjg_auditor;
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
RESET ROLE;
-- Runtime principals (sjg_audit_writer, sjg_auditor) receive schema/table privileges only in the post-grant migration.
