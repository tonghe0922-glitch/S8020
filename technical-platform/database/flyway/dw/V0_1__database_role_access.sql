...
DO $$
BEGIN
  IF current_database() <> 'sjg_dw' THEN
    RAISE EXCEPTION 'PHASE-03 migration expected database sjg_dw, got %', current_database();
  END IF;
END
$$;
...
GRANT CONNECT ON DATABASE sjg_dw TO sjg_migration;
GRANT CONNECT ON DATABASE sjg_dw TO sjg_dw_writer;
GRANT CONNECT ON DATABASE sjg_dw TO sjg_dw_reader;
REVOKE CREATE ON SCHEMA public FROM PUBLIC;
RESET ROLE;
-- Runtime principals (sjg_dw_writer, sjg_dw_reader) receive schema/table privileges only in the post-grant migration.
